	...
	for (const lwSurface* surface = lwo->surf; surface; surface = surface->next)
	{
		...
		if (texDiffuse)
		{
			...
			if (clip)
			{
				...
				material.setDiffuseMap(Material::Map(textureName, outModel->addUniqueTexCoordChannel(channel), true));
		...
		if (texSpecular)
		{
			...
			{
				std::wstring textureName = fixTextureFileName(mbstows(clip->source.still.name));
				material.setSpecularMap(Material::Map(textureName, outModel->addUniqueTexCoordChannel(channel), false));
			}
			else
		...
		if (texTransparency)
		{
			...
			{
				std::wstring textureName = fixTextureFileName(mbstows(clip->source.still.name));
				material.setTransparencyMap(Material::Map(textureName, outModel->addUniqueTexCoordChannel(channel), false));
			}
			else
		...
		if (texEmissive)
		{
			...
			{
				std::wstring textureName = fixTextureFileName(mbstows(clip->source.still.name));
				material.setEmissiveMap(Material::Map(textureName, outModel->addUniqueTexCoordChannel(channel), false));
			}
			else
		...
		if (texReflective)
		{
			...
			{
				std::wstring textureName = fixTextureFileName(mbstows(clip->source.still.name));
				material.setReflectiveMap(Material::Map(textureName, outModel->addUniqueTexCoordChannel(channel), false));
			}
			else
		...
		if (texBump)
		{
			...
			{
				std::wstring textureName = fixTextureFileName(mbstows(clip->source.still.name));
				material.setNormalMap(Material::Map(textureName, outModel->addUniqueTexCoordChannel(channel), false));
			}
			else
